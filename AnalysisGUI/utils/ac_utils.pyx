...
cdef ndarray parallel_bandpass3D(ndarray[double, ndim=3] series, double framerate, double start = 0.1,double end = 6, int num_workers=8):
    ...
    """
    cdef:
        ndarray[double, ndim=3] filtered_series
        ndarray[double,ndim=2] sos
        ndarray[double,ndim=1] freqs = np.array([start,end],dtype=np.float64)
    ...
    # Create filter
    sos = butter(4, btype='bandpass', fs=framerate, Wn=freqs, output='sos')
...
cdef ndarray filter_chunk(ndarray[double,ndim=3] chunk,ndarray[double,ndim=2] sos):
    
    cdef ndarray[double, ndim=3] filtered_chunk = sosfiltfilt(sos, chunk, axis=0).astype(np.float64)
    return filtered_chunk



    
cdef tuple figure_limits(ndarray[double, ndim=3] series, double framerate, int start, int end):
    ...
    """
    cdef:
        ndarray[double, ndim=1] time = np.linspace(0, series.shape[0]/framerate, series.shape[0])
        ndarray[double, ndim=1] meanseries = np.mean(np.mean(series, axis=1), axis=1)
        ndarray[long, ndim=1] defaults = np.array([start, end], dtype=int)
        ndarray[double, ndim=1] diff
        ndarray[long, ndim=1] outliers
        ndarray[long, ndim=1] outlierdiffs
        ndarray[long, ndim=1] selectedrange
        double nperiods
        int i
...
cdef interpolate_chunk(tuple chunk_data):
    ...
    cdef:
        ndarray[double,ndim=3] series_chunk = chunk_data[0]
        ndarray[double,ndim=1] real_t = chunk_data[1]
        ndarray[double,ndim=1] synthetic_t = chunk_data[2]
        ndarray[double,ndim=3] interp_chunk
    cdef object spl = interpolate.PchipInterpolator(real_t, series_chunk, axis=0)
    
    interp_chunk = spl(synthetic_t)
    return interp_chunk

cdef parallel_interpolate3D(ndarray[double, ndim=3] series, double framerate, int num_workers=8):
    ...
    cdef:
        ...
        ndarray[double, ndim=3] interpseries
        ndarray[double, ndim=1] synthetic_t
        ndarray[double, ndim=1] real_t
        int height = series.shape[1]
        int chunk_size
        int start_row, end_row
        list chunks = []
...
cdef process_fft_chunk(tuple chunk_data):
    ...
    cdef:
        ...
        ndarray[double,ndim=3] series_fft_abs
        ndarray[double,ndim=1] series_frequencies
        int good_freq
        int nfft
        ndarray[double,ndim=2] ACarray_chunk
...
cdef parallel_getACimage(ndarray[double, ndim=3] series, double framerate, double frequency, int num_workers=8):
    ...
    cdef:
        ...
        int chunk_size
        int start_row, end_row
        ndarray[double, ndim=2] full_ACarray
        list chunks = []
        int i
    # Split the image into horizontal chunks
    chunk_size = max(1, height // num_workers)
...
cpdef get_AC_data(ndarray[double, ndim=3] images, double framerate=16.7, double tolerance=0.2, 
    ...
    """
    cdef:
        ndarray[double, ndim=3] series = images.copy()
        ndarray[double, ndim=2] DCarray
        ndarray[double, ndim=2] ACarray
        ndarray[double, ndim=1] time
        ndarray[double, ndim=1] meanseries
        int newlastindex
        double tic,toc
    ...
    DCarray = np.mean(series, axis=0,dtype = np.float64)
    ...
    time = np.linspace(0, series.shape[0]/framerate, series.shape[0],dtype = np.float64)
