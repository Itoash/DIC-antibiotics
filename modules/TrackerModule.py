...
def apply_warping_fullview(images, warp_stack, PATH=None):
    ...
    image_0 = np.zeros((images[0].shape[0]+top+bottom,images[0].shape[1]+left+right))
    image_0[top:-bottom,left:-right] = images[0]
    imgs = [image_0]
    for i, img in enumerate(images[1:]):
        H_tot = next(H)+np.array([[0,0,left],[0,0,top],[0,0,0]])
        img_warp=cv2.warpPerspective(img,H_tot,(img.shape[1]+left+right,img.shape[0]+top+bottom),flags = cv2.INTER_NEAREST)
        if not PATH is None:
...
def process_frame(currentimage,nextimage,imageid):
    ...
    for col in range(1,int(np.max(currentimage))+1):
        ...
        if np.sum(filtered) > 1:
            ...
            contour,hierarchy = cv2.findContours(filtered.astype('uint8'),cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
            contour = contour[0]
    ...
    #framedata = {'frame':imageid,'overlaps':overlapdict,'COMs':COMdict,'angles':angledict,'majors':majordict,'minors':minordict,'areas':areadict,'convex':convexdict}
...
def multiprocessframes(imagestack):
    ...
    opstack.append(np.zeros_like(opstack[0])) # zero-pad stack to add data about last framw w/o overlaps
    import concurrent.futures
    with concurrent.futures.ProcessPoolExecutor() as executor:
        stackdata = list(executor.map(process_frame,opstack[:-1],opstack[1:],np.arange(0,len(imagestack))))
    COMlist = []
    for el in stackdata:
    ...
    def removeNode(self,node,debug = False):
        ...
        if node.frame == self.frame-1:
            ...
            if self.backwardsoverlaps is not None:
                coords = np.argwhere(self.backwardsoverlaps == name) # always 2D array, so need second dimension
                if len(coords) != 0:
                   
                    self.backwardsoverlaps = np.delete(self.backwardsoverlaps,(coords[0,1]),axis = 1)
                coords = np.argwhere(self.backwardsoverlaps[0,:] > name) # always 2D array, so need second dimension
                if len(coords) != 0:
...
class imagenetwork:
    def __init__(self,images,nodes,stackdata):
    ...
    def findchildren(self,n):
        ...
        childrenlist = [el for el,w in zip(children,weights) if w >self.similarity_threshold]
    ...
    def deletenode(self,name,frame):
        ...
        chosennode = [n for n in self.nodes if n.name == name and n.frame == frame][0]
...
class basicplot: # fucked up GUI
    ...
    def mouse_event(self,event):
        print('\n x: {} , y: {},imvalue {}'.format(event.xdata, event.ydata,self.image[int(event.ydata),int(event.xdata)]))
    ...
    def CLImode(self):
        ...
        for col in range(1,int(np.max(self.image))+1):
            ...
            print('\n')
        userinput = input("Enter label(s) to be deleted (separated by commas), or elimination criteria (syntax: a/x/y </> value): ")
        # make a switch case
        ...
        match userinput:
            ...
            case userinput if 'a<' in str(userinput) or 'a>' in str(userinput):
                ...
                if symbol == '>':
                    ...
                    labelarray = [self.colareas.index(i)+1 for i in self.colareas if i > limit]
    ...
    def mouse_event_draw(self, event):
        ...
        if event.button == 1:  # Left mouse button clicked
            if not self.drawing:  # Start drawing
                self.drawing = True
                self.xs = [event.xdata]  # Start new drawing at the click position
                self.ys = [event.ydata]
            else:  # Continue drawing while dragging
                self.xs.append(event.xdata)
                self.ys.append(event.ydata)
                self.update_drawing()
        elif event.button == 3:  # Right mouse button clicked (optional, to stop drawing)
            self.drawing = False
            print("Stopped drawing")
    
    def update_drawing(self):
        if self.drawing:
            polygonx,polygony = polygon(np.asarray(self.xs).astype(int),np.asarray(self.ys).astype(int))
    ...
    def drawmode(self):
        ...
        plt.imshow(self.image)
        self.fig.canvas.mpl_connect('button_press_event', self.mouse_event_draw)
        self.fig.canvas.mpl_connect('motion_notify_event', self.mouse_event_draw)
        self.fig.canvas.mpl_connect('button_release_event', self.mouse_event_release)
        ...
        userinput = input("Select a color/lw , or press 0 to exit,or p to pause/unpause drawing: ")
        match userinput:
            case userinput if str(userinput) in ['CLI','draw','erase','fill']:
    ...
    def erasemode(self):
        ...
        self.disableerasing = False
        plt.imshow(self.image)
        self.fig.canvas.mpl_connect('button_press_event', self.mouse_event_erase)
...
def checkifunder(array,name):
    if np.any(array>1):
        ...
        print("Found value((s) "+str(np.where(array>1))+" at positions "+str(np.argwhere(array>1)))
...
def evaluateframe(frame,framedict1,framedict2,distancematrix,ind):
    ...
    distancelist = [distancematrix[i,:] for i in range(distancematrix.shape[0])]
    ...
    with concurrent.futures.ThreadPoolExecutor() as exe:
        frametrack = list(exe.map(evaluatecandidates,distancelist,overlaplist,anglematrix,areamatrix,armatrix,np.arange(1,np.max(frame)+1),[ind for i in range(len(distancelist))]))
        #index 0 is label 1, and so on
        
    # index = [ind for i in range(len(distancelist))]
    # colors = np.arange(1,np.max(frame)+1)
    # for d,o,an,ar,i,c in zip(distancelist,overlaplist,anglematrix,areamatrix,index,colors):
    #     el = evaluatecandidates(d,o,an,ar,i,c)
    #     frametrack.append(el)
    return frametrack
...
def main():
    ...
    # print('got here') 
    # frame = 0
    # newstack = printimgswithdata2(processedimgs[frame:frame+2],stackdata[frame:frame+2])
    
    # for i in newstack:
    #     plt.imshow(i)
    #     plt.show(block=False)
    
    # confirm = input("Printed images. Proceed with tracking? (Y/N)")
    
    # if confirm == 'Y' or confirm =='y':
    #     distancelist = [dist[frame][i,:] for i in range(dist[frame].shape[0])]
    #     angleslist3 = [stackdata[frame]['angles'][i] for i in stackdata[frame]['angles'].keys()]
        
    #     angleslist4 = [stackdata[frame+1]['angles'][i] for i in stackdata[frame+1]['angles'].keys()]
    #     arealist3 = [stackdata[frame]['areas'][i] for i in stackdata[frame]['areas'].keys()]
    #     arealist4 = [stackdata[frame+1]['areas'][i] for i in stackdata[frame+1]['areas'].keys()]
    #     overlaplist = [stackdata[frame]['overlaps'][i] for i in stackdata[frame]['overlaps'].keys()]
    #     anglematrix = ratiomatrix(np.asarray(angleslist3),np.asarray(angleslist4))
    #     anglematrix = [anglematrix[i,:] for i in range(anglematrix.shape[0])]
    #     areamatrix = ratiomatrix(np.asarray(arealist3),np.asarray(arealist4))
    #     areamatrix = [areamatrix[i,:] for i in range(areamatrix.shape[0])]
    ...
    # else:
    #     return stackdata,COMlist,dist
    
    
    # with concurrent.futures.ProcessPoolExecutor() as executor:
    #     labelcandidates = list(executor.map(evaluatecandidates,distancelist,overlaplist,anglematrix,areamatrix))
    # labelcandidates = []
    # for d,o,ang,ar in zip(distancelist,overlaplist,anglematrix,areamatrix):
    #     el = evaluatecandidates(d,o,ang,ar)
    #     labelcandidates.append(el)
    
        
        
    # processedimgs.append(processedimgs[-1])
    # stackdata.append(stackdata[-1])
    # dist.append(dist[-1])
    
    # with concurrent.futures.ProcessPoolExecutor() as executor:
    #     tracklist = list(executor.map(evaluateframe,processedimgs[:-1],stackdata[:-1],stackdata[1:],dist[:-1],np.arange(0,len(dist))))
    tracklist = []
    stackdata.append(stackdata[-1])
    ...
    # for i,el in enumerate(labelcandidates[:]):
    #     print("Best candidates for label "+str(i+1)+" are "+str(el[0])+", with scores: "+str(el[1]))
    newnodes = tracklist.copy()
    
    nuimg = filterimagebynodes(processedimgs[:-1],newnodes)
    # for lis in newnodes:
    #     print(lis[3].name)
    #     print(lis[3].children)
    # for n,i in zip(nuimg,processedimgs[:-1]):
...
if __name__ == '__main__':
    x,d,t,n,net = main()
    featurearray = np.zeros((len(net.nodes),2))
    nodeindexes = {}
    for i,node in enumerate(net.nodes):
        
        
        featurearray[i,0] = node.area
        featurearray[i,1] = node.minor
        
        nodeindexes.update({i:node})
    for i in range(2):
        featurearray[:,i] -= np.mean(featurearray[:,i])
        featurearray[:,i] /= np.std(featurearray[:,i])
