...
def obtain_network(imgs,scorethreshold = 0.5):
    ...
    for i in range(len(dist)):
        tracklist.append(evaluateframe(processedimgs[i],stackdata[i],stackdata[i+1],dist[i],i))
    stackdata = stackdata[:-1]
    network = imagenetwork(processedimgs,tracklist,stackdata,scorethreshold=scorethreshold)
    return network
...
class imagenetwork:
    ...
    def unlinknode(self,nodepos):
        node = [n for n in self.nodes if n.name == nodepos[1] and n.frame == nodepos[0]][0]
        for n in self.nodes:
            n.removeNode(node)
        node.likelychildren = []
        node.likelyparent = []
    ...
    def linknodes(self,parentcoords,childcoords):
        parent = [n for n in self.nodes if n.name == parentcoords[1] and n.frame==parentcoords[0]][0]
        child = [n for n in self.nodes if n.name == childcoords[1] and n.frame==childcoords[0]][0]
