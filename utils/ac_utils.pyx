...
from scipy.signal import detrend, butter, sosfiltfilt, fftconvolve, hilbert
import concurrent.futures
import os
import time as tm
def parallel_bandpass3D(np.ndarray[double, ndim=3] series, double framerate,np.ndarray[double,ndim=1] frequencies=np.array([0.1, 6]), int num_workers=8):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        results = list(executor.map(filter_chunk, chunks,framerates,cutoffs))
...
def filter_chunk(np.ndarray[double,ndim=3] chunk,double framerate,np.ndarray[double,ndim=1] frequencies):
...
def applybandpass(np.ndarray[double, ndim=3] series, double framerate, np.ndarray[double,ndim=1] frequencies=np.array([0.1, 6])):
    """Apply bandpass filter to time series data.
    
    Args:
        series: Input 3D array with dimensions (time, height, width)
        framerate: Sampling rate in Hz
        frequencies: Tuple with (low_cutoff, high_cutoff) frequencies in Hz
        
    Returns:
        Filtered data with same shape as input
    """
    sos = butter(4, btype='bandpass', fs=framerate, Wn=frequencies, output='sos')
    cdef np.ndarray[double, ndim=3] filtereddata = sosfiltfilt(sos, series, axis=0)
    return filtereddata
    
def figure_limits(np.ndarray[double, ndim=3] series, double framerate, int start, int end):
...
def interpolate_chunk(chunk_data):
    """Process a single chunk for interpolation"""
    series_chunk, real_t, synthetic_t = chunk_data
    spl = interpolate.PchipInterpolator(real_t, series_chunk, axis=0)
    return spl(synthetic_t)

def parallel_interpolate3D(np.ndarray[double, ndim=3] series, double framerate, int num_workers=8):
...
def process_fft_chunk(chunk_data):
    """Process FFT on a single spatial chunk"""
    series_chunk, framerate, frequency, N = chunk_data
    ...
    ACarray_chunk = np.mean(series_fft_abs[good_freq:good_freq+1, :, :], axis=0)
    return ACarray_chunk

def parallel_getACimage(np.ndarray[double, ndim=3] series, double framerate, double frequency, int num_workers=8):
...
def get_AC_data(np.ndarray[double, ndim=3] images, double framerate=16.7, double tolerance=0.2, 
